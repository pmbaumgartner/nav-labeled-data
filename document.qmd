...
(My goal is to illustrate a set of tools that you might not be familiar with and illustrate how they might work together on a typical project. Though these methods are presented in the context of a "project", I hesitate to call this a workflow as you may not use every method for every project. I will not be diving deep into the math or history of any method, but I will give general context on each one. )

Natural Language Processing (NLP) projects often depend on the availability and quality of labeled data for supervised learning. Recently, the data-centric AI movement [@WhyItTime2023] has emerged as a guiding philosophy for projects, which has driven the development new methods that address various tasks within the standard supervised learning workflow. This paper aims to provide an overview of these new tools by working through an example project, illustrating how these methods can help practitioners optimize their data collection process. The topics covered include sentence embeddings, submodular sampling, creating better annotation experiences, using inter-annotator agreement, few-shot learning, and confident learning.

We'll use HappyDB [@asai2018happydb] as an example dataset for this. HappyDB is "a collection of happy moments described by individuals experiencing those moments." It was collected via crowd-sourcing on Amazon Mechanical Turk, where each worker was presented with the following task:
...
![Performance measures from the HappyDB paper](table10.png){width=300}
...
display(df_annotated.info())
display(df_annotated.head())
```

## Embedding Documents & Sentence Embeddings

Embeddings transform text into fixed-size numerical vectors, have become a powerful tool for representing and comparing sentences in NLP tasks. These embeddings capture semantic information and enable efficient comparisons between sentences, thereby proving useful in various NLP tasks such as sentiment analysis, document classification, and machine translation.

Metaphorically I think of embeddings as assigning an address to each _thing_ you're embedding, such that _things_ that are similar will have addresses next to each other. 

Semantically meaningfull embeddings were popoularized first at the word-level with the word2vec algorithm [@mikolov2013distributed]. 

 Several approaches to generating sentence embeddings have been proposed, including:

1. **Averaging Word Embeddings**: This technique involves averaging pre-trained word embeddings such as Word2Vec, GloVe, or FastText to create a fixed-size sentence representation. Despite its simplicity, this approach has demonstrated meaningful results in numerous NLP tasks.
...
In the context of labeled data collection, sentence embeddings can significantly enhance the sampling and annotation process. For example, they can aid in identifying similar examples in a dataset, which in turn allows the creation of diverse and representative samples for annotation. Additionally, embeddings can facilitate active learning by selecting the most informative examples for annotation, optimizing the labeling effort and maximizing model performance.

### Preprocessing
The amount of preprocessing you want to do will depend on your data, but it is important to make sure it's consistent. For this case, we'll simply lowercase all of the texts. One thing you do **not** want to do is remove stopwords or lemmatize. When using pre-trained models, you want your texts to be as close to the training data as possible. 

```{python}
# | label: embedding-similarity
# | fig-subcap: "Similarity values of sentence-embedding pairs"
# | code-summary: "Code for Embedding  "
...
sample_sims_df.sort_values("sim", ascending=False).head(3).round(2)
```


## Dimensionality Reduction

Dimensionality reduction plays a crucial role in the analysis and visualization of high-dimensional sentence embeddings. By reducing the number of dimensions while preserving the underlying structure and relationships between data points, dimensionality reduction techniques can transform sentence embeddings into a more compact and interpretable representation.  These methods help in identifying patterns, clusters, or relationships in the data, which can inform decisions related to data sampling, annotation, and model training. 

We'll be using TSNE (t-distributed stochastic neighbor embedding).


```{python}
# | label: umap-embeddings
# | fig-subcap: "A plot of HappyDB text embeddings run through TSNE with cosine similarity"
...
```{python}
# | label: umap-submodular-selection
...
display(selected_sorted.head(5))
display(Markdown("**Last 5**"))
display(selected_sorted.tail(5))
```
...
Let's assume we're interested in this task of predicting which of the six categories a happy moment fits into. Let's pretend we don't have ANY labeled data and we want to aquire some. 
...
![SetFit Performance on Customer Reviews (Sentiment)](https://huggingface.co/blog/assets/103_setfit/setfit_curves.png)

!["SetFit Architecture"](https://github.com/huggingface/setfit/blob/main/assets/setfit.png?raw=true)
...
model = SetFitModel.from_pretrained("sentence-transformers/paraphrase-MiniLM-L12-v2")
...
It will be helpful to have a model that can train quickly, since we have to train 5 instances of it. We'll choose a `FastText` model, which uses word and sub-word embeddings and then has a simple classifier head that averages all embeddings over a document [@joulin2016bag].^[`FastText` is also a good baseline choice and is particularly effective if your texts have inconsistency in spelling.]

Issues: ontological (subset/labels not mutually exclusive), multilabel (multiple things in the ), errors.
